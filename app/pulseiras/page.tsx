...
export default async function PulseirasPage() {
  const products = (await getProducts(false)).filter(
    (product) =>
      product.categories?.name?.toLowerCase() === "pulseiras"
  );
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-12 text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm text-neutral-500">
            Encontre a pulseira ideal para completar sua produção.
          </p>
        </div>

        {products.length === 0 ? (
          <div className="py-20 text-center text-neutral-500">
            Nenhuma pulseira disponível no momento.
          </div>
        ) : (
